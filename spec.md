...
A container specification defines the runtime configuration of a single container. It is the
common set of attributes shared by anything that runs a container: [services](05-services.md),
[jobs](jobs.md), and run-to-completion init containers ([`pre_start` hooks](05-services.md#pre_start)).

Attributes that only make sense for an orchestrated workload (build, ports, health checks,
dependencies, ...) are not part of the container specification: they are defined by the
[Workload specification](workload_spec.md) shared by services and jobs.
