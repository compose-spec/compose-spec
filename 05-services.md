...
Note that support for the `uid`, `gid`, and `mode` attributes is not implemented in Docker Compose when the secret source 
is a [`file`](09-secrets.md), as bind-mount used under the hood doesn't allow uid remapping.
